...
Client.prototype.connect = function connect() {
  ...
  function setupClient(cb) {
    ...
    socket.bail = function bail(err) {
      socket.destroy();
      cb(err || new Error('client error during setup'));
    };
    // Work around lack of close event on tls.socket in node < 0.11
    ((socket.socket) ? socket.socket : socket).once('close', socket.bail);
    socket.once('error', socket.bail);
    socket.once('end', socket.bail);
    socket.once('timeout', socket.bail);
  ...
  function postSetup() {
    // Work around lack of close event on tls.socket in node < 0.11
    ((socket.socket) ? socket.socket : socket).removeListener('close', socket.bail);
    socket.removeListener('error', socket.bail);
    socket.removeListener('end', socket.bail);
    socket.removeListener('timeout', socket.bail);
